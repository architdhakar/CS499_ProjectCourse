...
def extract_features(subgroup):
    ages = np.array([row.get('age', 0) for row in subgroup]).reshape(-1,1)
    hours = np.array([row.get('hours.per.week', 0) for row in subgroup]).reshape(-1,1)

    workclass = [row.get('workclass', 'unknown') for row in subgroup]
    education = [row.get('education', 'unknown') for row in subgroup]

    le1 = LabelEncoder()
    le2 = LabelEncoder()

    work_enc = le1.fit_transform(workclass).reshape(-1,1)
    edu_enc = le2.fit_transform(education).reshape(-1,1)

    X = np.hstack([ages, hours, work_enc, edu_enc])
    X = StandardScaler().fit_transform(X)

    return X

def tfair_select(subgroup, formatter, label_fn, k):
    scored = []

    for row in subgroup:
        demo = formatter(row) + " " + label_fn(row)

        # simulate small prompt
        temp_prompt = demo

        probs = get_llm_probabilities(temp_prompt + "\nInput: N/A\nIncome:")
        entropy = -sum(p * math.log(p + EPS) for p in probs)

        scored.append((entropy, row))

    scored.sort(key=lambda x: x[0], reverse=True)
    return [r for _, r in scored[:k]]

def gfair_select(subgroup, formatter, label_fn, k):
    remaining = subgroup.copy()
    selected = []
    current_prompt = ""
    ...
    while remaining and len(selected) < k:
        ...
        for row in remaining:
            ...
            temp_prompt = demo if current_prompt == "" else demo + "\n" + current_prompt

            probs = get_llm_probabilities(temp_prompt + "\nInput: N/A\nIncome:")
            score = -sum(p * math.log(p + EPS) for p in probs)
        ...
        current_prompt = formatter(best) + " " + label_fn(best) + "\n" + current_prompt
...
def compute_individual_fairness(demos, data, formatter, label_fn, num_pairs=100):
    ...
    def similarity(r1, r2):
        ...
        if abs(r1['age'] - r2['age']) <= 5:
            score += 1
        if r1['education'] == r2['education']:
            score += 1
        if r1['occupation'] == r2['occupation']:
            score += 1
        if abs(r1['hours.per.week'] - r2['hours.per.week']) <= 5:
            score += 1
    ...
    for _ in range(num_pairs):
        ...
        if similarity(r1, r2) < 0.75:
            continue

        p1 = predict_with_llm("\n".join([
            formatter(d) + " " + label_fn(d) for d in demos
        ]), formatter, r1)
...
if __name__ == "__main__":
    ...
    # Build clustered candidate pool
    clustered_pool = []
    for subgroup in fcg.subgroups.values():
        clustered_pool.extend(subgroup)

    print(f"\nClustered pool size: {len(clustered_pool)}")
    ...
    for strategy in ["s1", "s2", "s3"]:
        methods[f"FCG-{strategy}"] = fcg.step3_select_demonstrations(strategy)

    # ---------------- T-Fair ----------------
    methods["T-Fair"] = tfair_select(clustered_pool, formatter, label_fn, K)

    # ---------------- G-Fair ----------------
    methods["G-Fair"] = gfair_select(clustered_pool, formatter, label_fn, K)

    # ---------------- Random (IMPROVED) ----------------
    def random_avg(pool, k, trials=5):
        all_sets = []
        for _ in range(trials):
            demos = random.sample(pool, min(k, len(pool)))
            all_sets.append(demos)
        return all_sets

    random_sets = random_avg(clustered_pool, K)

    # Evaluate all methods
    for name, demos in methods.items():
        print(f"\n--- {name} ---")
        ...
        fair_metrics = calculate_fairness_metrics(
            predictions, true_labels, sensitive_attrs, label_fn
        )

        indiv_fair = compute_individual_fairness(
            demos, test_data, formatter, label_fn
        )

        print(f"Accuracy: {accuracy:.4f}")
        print(f"DP Ratio: {fair_metrics['dp_ratio']:.4f}")
        print(f"EO Ratio: {fair_metrics['eo_ratio']:.4f}")
    ...
    for demos in random_sets:
        ...
        acc = sum(p == t for p, t in zip(predictions, true_labels)) / len(predictions)
        fair_metrics = calculate_fairness_metrics(predictions, true_labels, sensitive_attrs, label_fn)
        indiv = compute_individual_fairness(demos, test_data, formatter, label_fn)

        accs.append(acc)
        dps.append(fair_metrics['dp_ratio'])
        eos.append(fair_metrics['eo_ratio'])
        ifs.append(indiv)

    print(f"Accuracy: {np.mean(accs):.4f}")
    print(f"DP Ratio: {np.mean(dps):.4f}")
    print(f"EO Ratio: {np.mean(eos):.4f}")
    print(f"Individual Fairness: {np.mean(ifs):.4f}")
